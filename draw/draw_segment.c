...
/*   Updated: 2016/03/02 17:47:10 by alhote           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		c2(t_segment *seg, t_world *w)
{
	...
	while (i <= fabs(seg->p[1]->y2d - seg->p[0]->y2d))
	{
		...
		if (!safe_pixel_put(w, pos[0], pos[1], seg->p[0]->color * w->h / 1 + seg->p[1]->color * w->h / 1))
			return (0);
		++i;
	}
	return (0);
...
static int		c1(t_segment *seg, t_world *w)
{
	...
	while (i <= fabs(seg->p[1]->x2d - seg->p[0]->x2d))
	{
		...
		if (!safe_pixel_put(w, pos[0], pos[1], seg->p[0]->color * w->h * 10 / 1 + seg->p[1]->color * w->h * 10 / 1))
			return (0);
		++i;
	}
	return (0);
...
int				draw_segment(t_segment *seg, t_world *w)
{
	...
	{
		safe_pixel_put(w, seg->p[0]->x2d, seg->p[0]->y2d, seg->p[0]->color);
